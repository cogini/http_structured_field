defmodule ParserTest do
  ...
  describe "binary" do
    test "binary" do
      assert {:ok, {:binary, "pretend this is binary content."}} ==
               Parser.parse(":cHJldGVuZCB0aGlzIGlzIGJpbmFyeSBjb250ZW50Lg==:")

      assert {:error, "Invalid base64"} ==
               Parser.parse(":cHJldGVuZCB0aGlzIGlzIGJpbmFyeSBjb250ZW50Lg=:")
    end
  end

  test "parameters" do
    assert {:ok, {:integer, 1, [{"a", {:boolean, true}}, {"b", {:boolean, false}}]}} ==
             Parser.parse("1; a; b=?0")

    # Longer name
    assert {:ok, {:integer, 1, [{"abc", {:boolean, true}}, {"b", {:boolean, false}}]}} ==
             Parser.parse("1; abc; b=?0")
  end
  ...
  test "inner list" do
    ...
    assert {:ok,
            [
              {:inner_list, [{:string, "foo", [{"a", {:integer, 1}}, {"b", {:integer, 2}}]}],
               [{"lvl", {:integer, 5}}]},
  ...
  test "list" do
    # From RFC
    assert {:ok, [{:token, "sugar"}, {:token, "tea"}, {:token, "rum"}]} ==
             Parser.parse("sugar, tea, rum")

    assert {:ok,
            [
              {:token, "abc",
               [{"a", {:integer, 1}}, {"b", {:integer, 2}}, {"cde_456", {:boolean, true}}]},
              {:inner_list, [{:token, "ghi", [{"jk", {:integer, 4}}]}, {:token, "l"}],
